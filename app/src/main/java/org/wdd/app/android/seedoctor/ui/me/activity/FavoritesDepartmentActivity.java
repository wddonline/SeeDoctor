...
import org.wdd.app.android.seedoctor.ui.me.adapter.FavoritesDepartmentAdapter;
import org.wdd.app.android.seedoctor.ui.me.presenter.FavoritesDepartmentPresenter;
...
public class FavoritesDepartmentActivity extends BaseActivity implements FavoritesDepartmentAdapter.FavoritesDepartmentCallback {
    ...
    private List<FavoritesDepartmentAdapter.DepartmentFavorites> departments;
    private FavoritesDepartmentPresenter presenter;
    private FavoritesDepartmentAdapter adapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_favorites_department);
        initData();
        initTitles();
        initViews();
    }
    ...
    private void initData() {
        presenter = new FavoritesDepartmentPresenter(this);
    }
    ...
    private void initViews() {
        recyclerView = (RecyclerView) findViewById(R.id.activity_favorites_department_recyclerview);
        recyclerView.addItemDecoration(new LineDividerDecoration(this, LinearLayoutManager.VERTICAL));
        recyclerView.setLayoutManager(new LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false));
        recyclerView.setHasFixedSize(true);
        checkBox = (CheckBox) findViewById(R.id.activity_favorites_department_all);
        loadView = (LoadView) findViewById(R.id.activity_favorites_department_loadview);
            ...
            public boolean onMenuItemClick(MenuItem item) {
                switch (item.getItemId()) {
                    case R.id.menu_favorites_cancel:
                        if (adapter.getMode() == FavoritesDepartmentAdapter.Mode.Select) {
                            cancelSelectMode();
                        }
                        return true;
    ...
    private void cancelSelectMode() {
        adapter.setMode(FavoritesDepartmentAdapter.Mode.Normal);
        toolbar.setNavigationIcon(R.mipmap.back);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        presenter.getFavoritesDepartments();
        return true;
    }
    ...
    public void bindDepartmentListViews(List<DbDepartment> data) {
        departments = new ArrayList<>();
        for (DbDepartment doctor : data) {
            departments.add(new FavoritesDepartmentAdapter.DepartmentFavorites(false, doctor));
        }
        adapter = new FavoritesDepartmentAdapter(this, departments);
    ...
    public void onBackPressed() {
        if (adapter != null && adapter.getMode() == FavoritesDepartmentAdapter.Mode.Select) {
            cancelSelectMode();
            return;
        }
        super.onBackPressed();
